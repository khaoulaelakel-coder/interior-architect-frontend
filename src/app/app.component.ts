...
export class AppComponent {
  ...
  constructor() {
    ...
    this.hasSeenVideo = localStorage.getItem('videoSeen') === 'true';
    console.log('AppComponent: Initialized, appReady =', this.appReady, 'hasSeenVideo =', this.hasSeenVideo);

    // For testing: Uncomment the line below to reset video flag
    // this.resetVideoFlag();
  }

  onAppReady(videoPath: string) {
    console.log('App: Video ready event received with path:', videoPath);
    console.log('App: Setting appReady to true and marking video as seen');
    this.appReady = true;
    this.hasSeenVideo = true;

    // Store in localStorage so video won't show again in this session
    localStorage.setItem('videoSeen', 'true');

    console.log('App: appReady is now', this.appReady, 'hasSeenVideo is now', this.hasSeenVideo);
  }

  // Method to reset video flag for testing (uncomment in constructor to use)
  resetVideoFlag() {
    localStorage.removeItem('videoSeen');
    this.hasSeenVideo = false;
    this.appReady = false;
    console.log('Video flag reset - video will show again on next page load');
  }
}
